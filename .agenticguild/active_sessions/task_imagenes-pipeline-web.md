...
<implementation_plan>
  ...
  <step id="2" status="pending">Linkear la regla desde `docs/core/SYSTEM_ARCHITECTURE.md` en la sección de imágenes/Active Storage para que sea parte explícita de las fronteras del sistema.</step>
  <step id="3" status="pending">Estándar de pipeline: definir un único punto (helper + servicio) para pedir/renderizar variantes `thumb/list/detail` y evitar que cada vista/controller invente tamaños/formats. (Mantener HTML-first; nada de lógica en controllers.)</step>
  <step id="4" status="pending">Implementar normalización WebP en variantes Active Storage para todos los attachments existentes que se usan en UI; asegurar que la app siempre renderiza variantes (no el original) salvo fallback explícito.</step>
