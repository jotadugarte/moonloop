...
<implementation_plan>
  <step id="1" status="pending">Write a failing spec that captures the dish picker contract for menu slots (grouping by `meal_type`, group ordering with slot type first then canonical order, and filtering-by-name that preserves groups). Tag with [REQ-MENU-001].</step>
