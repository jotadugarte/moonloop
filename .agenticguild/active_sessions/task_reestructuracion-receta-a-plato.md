...
<implementation_plan>
  ...
  <step id="2" status="complete">Migración de esquema: `recipes` → `dishes` (o equivalente), `menu_entries.recipe_id` → `dish_id`, FKs e índices; **sin** lógica de app pesada en la migración (datos mecánicos solo; regla SYSTEM_ARCHITECTURE). Regenerar/actualizar `docs/core/SCHEMA_REFERENCE.md` cuando toque el flujo del repo.</step>
  <step id="3" status="complete">Renombrar modelo **`Recipe` → `Dish`**, asociaciones (`MenuEntry`, `User`, Active Storage), factories y referencias en servicios (`Menus::DuplicateDishForAdopter`, `UpsertEntry`, adopción/sync, admin). Specs y vistas alineados a `dishes_path` / `dish` params; Stimulus `dish-image-preview`.</step>
  <step id="4" status="complete">i18n: `activerecord.models.dish` (en/es), namespace **`dishes.*`**, **`menus.slots.dish_*`**, **`home.index.dishes`**, **`menus.index.dishes_link`**; formulario de plato usa **`forms.errors.header`** + `model_name.human`.</step>
  <step id="5" status="complete">`docs/core/SPEC.md` (glosario **Dish**, entidades, REQ-MENU-001/002/006, REQ-EXR-006, workflows, deep dive), `docs/core/DATA_FLOW_MAP.md` (UpsertEntry / `MenuEntry`), `docs/ROADMAP.md` (#52 hecho, Phase 4). RuboCop + RSpec en local del equipo.</step>
</implementation_plan>
