...
- **Plans (roadmap #61, REQ-PHS-001 / REQ-CAT-001):** public plan catalog copy under **`public_plans.*`** i18n; adopting a public plan duplicates menus and routines for the adopter via **`Plans::PopulateAssignmentsFromSource`**. Admins can **`PATCH /admin/plans/:id/revoke_public_share`**. Removed **`Programs::*`**, **`PhaseProgram`** / **`PhaseProgramAssignment`** models, **`MaterializePhaseProgramFacetDuration`**, and legacy **`phase_programs*`** controllers/views. UI lives under **`app/views/plans`** and **`app/views/plan_assignments`** with **`plans.*`** / **`plan_assignments.*`** i18n; catalog listing facets use **`Plan`** only (dropped **`PhaseProgram`** from **`LISTABLE_TYPES`**). **`Catalog::MaterializePlanFacetDuration`** clears duration fields when a plan has no segments. Service specs moved to **`spec/services/plans/`**; model specs renamed to **`plan_spec`** / **`plan_assignment_spec`**.
...
- **Menus (roadmap #59, REQ-MENU-001):** dish picker in `/menus/:id/edit` now shows a coherent closed state (“Sin plato” or the selected dish name), opens a grouped list predictably, supports Escape-to-close, and keeps Turbo slot re-renders consistent with full-page locals.
