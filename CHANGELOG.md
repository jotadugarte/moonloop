...
- **Plans (roadmap #61, REQ-PHS-001 / REQ-CAT-001):** public plan catalog copy under **`public_plans.*`** i18n; adopting a public plan duplicates menus and routines for the adopter via **`Plans::PopulateAssignmentsFromSource`** (parity with program adoption). Admins can **`PATCH /admin/plans/:id/revoke_public_share`** to remove a plan from the public catalog (`Admin::PlansController`). Removed legacy **`PublicPhaseProgramsController`** / **`public_phase_programs`** views and duplicate request specs; **`Plans::ApplyAdoptionSourceSync`** + **`Plans::AdoptionSyncStatus`** back `PlansController#accept_source_update`. Phase→plan snapshot builder lives in **`Plans::BuildFromPhases`** (replaces **`Programs::BuildFromPhases`**). Demo **`db:seed`** seeds a **`Plan`** (“Demo plan”) and **`spec/seeds/demo_dataset_spec`** applies it with **`Plans::ApplyToUser`**.
