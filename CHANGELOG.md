...
- Per-habit reminder delivery (**REQ-HAB-013**): after a successful `habit_reminder_events` insert, `Habits::ProcessHabitReminderForUserHabit` sends `HabitReminderMailer#notify` (`deliver_now`) when `reminder_email` and runs `Habits::DeliverHabitReminderWebPush` when `reminder_web_push` (`web-push` gem, VAPID via `config/initializers/habit_web_push.rb`, I18n payload, best-effort per subscription, delete stale rows on `WebPush::InvalidSubscription` / `ExpiredSubscription`). Idempotency relies on DB uniqueness + `RecordNotUnique` (model-level uniqueness validation removed from `HabitReminderEvent`). Docs updated: `SPEC.md`, `DATA_FLOW_MAP.md` §1.8, `ADRs/0001`, `SCHEMA_REFERENCE.md`, `ROADMAP.md` Done **#35**.
- **Per-habit reminders (MVP — partial):** per-habit reminder preferences on `UserHabit` (`reminder_enabled`, local `HH:MM` time-of-day interpreted in the user’s IANA timezone, `reminder_email` / `reminder_web_push` channel toggles), Solid Queue sweep (`Habits::SweepHabitRemindersJob` via `config/recurring.yml`) + processor (`Habits::ProcessHabitReminderForUserHabit`) with idempotent `habit_reminder_events` keyed by `(user_id, user_habit_id, local_date)` (**REQ-HAB-010**, **REQ-HAB-011**). Adds authenticated Web Push **subscription persistence** + unsubscribe endpoints (`WebPushSubscriptionsController`, `web_push_subscriptions`) (**REQ-HAB-012** persistence). Includes `HabitReminderMailer` + templates.
