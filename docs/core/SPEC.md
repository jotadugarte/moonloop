...
| `MENU` | Menus, **dishes** (plato in Spanish UI), phase plan — **implemented** (Phase 4) |
...
| Menu entry | At most one row per `(menu, weekday, meal_type)`; references optional **`Dish`** (`dish_id`) and/or `freeform_text` when the user allows freeform in profile. | `MenuEntry` |
| Dish | User-owned meal item (“**plato**” in Spanish UI): name, **`meal_type`**, optional **instructions** (preparation text; product copy may still say “receta” for that block), optional **ActiveStorage** image; may be marked publicly shareable for catalog browsing. Persisted in table **`dishes`**. | `Dish` |
...
- **User** (`users`)
  - `has_many :sessions`, `has_many :habit_categories`, `has_many :user_habits`, `has_many :weight_logs`, `has_many :menus`, `has_many :dishes`, `has_many :phase_assignments`, `has_many :phase_programs`, `has_many :phase_reminder_events`, `has_many :habit_reminder_events`, `has_many :web_push_subscriptions`; Phase 5 adds `has_many` exercise routines and routine week-range assignments (exact names per schema)
...
- **MenuEntry** (`menu_entries`)
  - `belongs_to :menu`, optional `belongs_to :dish` (FK **`dish_id`**)
  ...
  - Content: at least one of **`dish_id`** or `freeform_text` when persisted (subject to user preference)

- **Dish** (`dishes`)
  - `belongs_to :user`, `has_many :menu_entries`; optional `has_one_attached :image`
  - `meal_type` (canonical keys as for menu slots); optional **`instructions`**; `publicly_shareable`; admin may revoke sharing on the public dish catalog
...
| REQ-MENU-001 | Weekly **menu** plan: at most one persisted slot per `(menu, weekday, meal_type)`; slot holds a user-owned **`Dish`** and/or optional freeform text per profile preference; validations and Hotwire grid editor. | Implemented |
| REQ-MENU-002 | **`Dish`** model: name, **`meal_type`**, optional instructions, optional **ActiveStorage** image; in menu slots, fallback image by meal type when the dish has no image. Public catalog and owner CRUD under canonical routes (e.g. **`/platos`**, **`/public_dishes`**); legacy **`/recipes`** URLs redirect with **301**. | Implemented |
...
| REQ-MENU-006 | **Public menu catalog:** owner opt-in `publicly_shareable`; authenticated catalog index/show (no author email in HTML); adopt creates one copy per adopter per source with slot copy (**`Dish`** rows duplicated for the adopter where slots reference the author’s dishes) and sync fingerprint; explicit apply-update from source with stale detection; source deleted or made non-public yields unavailable copy UX; **`phase_assignments`** on the copy are unchanged on sync; admin revoke like public **dishes** / menus / routines. See **Acceptance criteria — REQ-MENU-006** below. | Implemented |
...
| REQ-EXR-006 | **Public exercise routine catalog:** owner opt-in `publicly_shareable`; authenticated catalog index/show (no author email in HTML); adopt creates one copy per adopter per source with line copy and sync fingerprint; explicit apply-update from source with stale detection; source deleted or made non-public yields unavailable copy UX; admin revoke like **dishes** / menus. See **Acceptance criteria — REQ-EXR-006** below. | Implemented |
...
6. **Moderation:** An **admin** (same **`MOONLOOP_ADMIN_EMAILS`** gate as **dishes** / menus) can **revoke** public sharing on a routine; it disappears from the public catalog.
...
| Q8 | **Public sharing** of routines (catalog, adoption, sync, moderation): **REQ-EXR-006**; parity intent with **Done #29** / public **dish** catalog (see **ROADMAP** **#30**). |
...
8. **Public menu catalog** (**REQ-MENU-006**) — Owner opts in with **`publicly_shareable`** on create/update; authenticated users browse **`public_menus`** and may **adopt** (one copy per source per adopter; **dishes** duplicated for the adopter where needed); **apply update** syncs slot content only; admin **revoke** removes the menu from the catalog.
...
- Phase 4 (Alimentación): `Menu`, `MenuEntry`, **`Dish`**, `PhaseAssignment`, `PhaseReminderEvent`; services under `app/services/menus/` and `app/services/phases/`; Turbo menu grid under `Menus::MenuEntriesController`; **`DishesController`** (owner CRUD, **`/platos`**), **`PublicDishesController`** (`/public_dishes`), **`PublicMenusController`**, adoption/sync services (`Menus::AdoptFromPublicCatalog`, `ApplyAdoptionSourceSync`, …); Solid Queue job `Phases::SweepPhaseStartRemindersJob` (see `config/recurring.yml`); admin moderation under `Admin::*` gated by `MOONLOOP_ADMIN_EMAILS` (**REQ-MENU-006**)
