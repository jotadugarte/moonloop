...
| REQ-HAB-010 | **Per-habit reminders — configuration:** a `UserHabit` may store an optional reminder configuration with `enabled` default **false**, a **local time-of-day** (interpreted in the user’s IANA timezone), and independent channel toggles **email** and **web push**. If enabled, at least one channel must be selected. Inactive habits are not eligible for reminders. | Planned |
| REQ-HAB-011 | **Per-habit reminders — idempotency:** a reminder sweep must be idempotent per `(user_id, user_habit_id, local_date)` so repeated runs do not duplicate the logical reminder for the same habit on the same user-local day. | Planned |
| REQ-HAB-012 | **Web Push subscriptions:** users may store one or more web push subscriptions (endpoint + keys) and unsubscribe cleanly; push delivery uses VAPID keys and is only attempted when the browser granted permission. | Planned |
| REQ-HAB-013 | **Habit reminder email:** when email channel is enabled, the system sends an I18n’d reminder email referencing the habit name and context, and respects idempotency and “already completed today” rules. | Planned |
