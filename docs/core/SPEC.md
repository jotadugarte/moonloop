...
| Habit completion | At most one persisted row per `(user_habit, local calendar day)` with status **done** or **failed**; **pending** means no row (or row removed). For habits with a non-**`none`** **habit metric kind**, the row also stores **day progress** toward the **daily target**; **explicit failed** from the user means the day is **not** fulfilled for streaks and reports even if progress is partial. Otherwise the day counts as **done** for streaks and reports when **day progress** ≥ **daily target**; persisted **`status`** stays **in sync** with those rules so **`Habits::Streak`**, **`Habits::MiDayStreakPrefetch`**, and reporting share one definition (**REQ-DAY-005**). | `HabitCompletion` / `habit_completions` |
