...
6. **Web Push:** authenticated clients **`POST /web_push_subscription`** / **`DELETE /web_push_subscription`** persist **`web_push_subscriptions`** (**REQ-HAB-012**). **`Habits::DeliverHabitReminderWebPush`** encrypts an I18n payload with the **`web-push`** gem using VAPID material from **`config/initializers/habit_web_push.rb`** (credentials / ENV); invalid endpoints (404/410 class errors from the gem) **delete** the stale subscription row.
