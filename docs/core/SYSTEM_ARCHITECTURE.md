...
- **Web Push delivery boundary:** Persisting `web_push_subscriptions` and exposing subscribe/unsubscribe endpoints must not be treated as “push is implemented”. Any change that introduces delivery (VAPID key management, payload encryption, retry/dead subscription handling, permission UX) must be documented explicitly (update ADRs and the relevant REQ in `SPEC.md`).
