...
- **Date of birth (HTTP params):** **`BirthDateTriplet`** (`app/controllers/concerns/birth_date_triplet.rb`) parses **`birth_year`**, **`birth_month`**, **`birth_day`** into a calendar **`Date`** or **`:incomplete`** / **`:invalid`** for **`RegistrationsController`** and **`ProfilesController`**. Impossible dates surface as **`date_of_birth: :invalid_calendar`** without persisting a constructed value.
