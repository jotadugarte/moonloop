...
RSpec.describe Plans::AdoptFromPublicCatalog do
  ...
  # [REQ-PHS-001]
  it "creates an adopted plan with duplicated menus, routines, and segments" do
    ...
    source = Plan.create!(user: author, name: "Template", publicly_shareable: true)
    ...
    copy = described_class.call(adopter: adopter, source: source, chosen_name: "  Mi plan  ")

    expect(copy.user_id).to eq(adopter.id)
    expect(copy.name).to eq("Mi plan")
    expect(copy.source_plan_id).to eq(source.id)
    expect(copy.adoption_catalog_origin_id).to eq(source.id)
    expect(copy.source_sync_fingerprint).to eq(Plans::ContentFingerprint.for_plan(source))
  ...
  it "reuses one menu copy when two segments share the same menu" do
    ...
    source = Plan.create!(user: author, name: "Dup menu", publicly_shareable: true)
    PlanAssignment.create!(plan: source, menu: menu, exercise_routine: r1, start_week: 1, end_week: 2)
    PlanAssignment.create!(plan: source, menu: menu, exercise_routine: r2, start_week: 3, end_week: 4)
    ...
    expect(copy.plan_assignments.pluck(:menu_id).uniq.size).to eq(1)
    expect(copy.plan_assignments.count).to eq(2)
  end
  ...
  it "raises already_adopted on a second adoption of the same source" do
    ...
    source = Plan.create!(user: author, name: "Once", publicly_shareable: true)
  ...
  it "raises not_public when the source is not shareable" do
    source = Plan.create!(user: author, name: "Privado", publicly_shareable: false)
  ...
  # [REQ-PHS-001]
  it "raises cannot_adopt_own when adopter owns the source plan" do
    source = Plan.create!(user: adopter, name: "Mine", publicly_shareable: true)
