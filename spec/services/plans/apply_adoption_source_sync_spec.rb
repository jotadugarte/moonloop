...
RSpec.describe Plans::ApplyAdoptionSourceSync do
  ...
  it "raises not_adopted_copy when there is no adoption metadata" do
    plan = Plan.create!(user: adopter, name: "X", publicly_shareable: false)

    expect do
      described_class.call(copy: plan)
  ...
  it "rebuilds segments and bumps fingerprint when the source template changed" do
    ...
    old_menu_id = copy.plan_assignments.sole.menu_id
    menu.menu_entries.sole.update!(freeform_text: "New")

    described_class.call(copy: copy.reload)

    copy.reload
    expect(copy.source_sync_fingerprint).to eq(Plans::ContentFingerprint.for_plan(source.reload))
    new_menu_id = copy.plan_assignments.sole.menu_id
